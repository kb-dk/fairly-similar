  ...
  public void createGui() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            try {
                ...
                if (match.equals(matchTypes[0])) {
                    ...
                    SortedSet<ImageNumberWithDistance> best
                            = HeuristicSolrUtil.findAndListBestHeuristicMarkers(Integer.parseInt(lineNumber), 200);
                    ...
                    SortedSet<ImageNumberWithDistance> best = HeuristicSolrUtil.findAndListBestHeuristicPredictions(
                    ...
                    SortedSet<ImageNumberWithDistance> best
                            = HeuristicSolrUtil.findAndListBestHeuristicMixed(Integer.parseInt(lineNumber), 200);
                    bestImages = best; // Now static variable that can be used by rendering frame
                }
                bestImages.stream()
                          .limit(10)
                          .forEach(image -> log.info("{}  {}", image.getLineNumber() + 1, image.getDistance()));
                createGallery();
    ...
    public void createGallery() {
        ...
        for (ImageNumberWithDistance current : bestImages) {
            JLabel label = new JLabel(current.getImageName() + " (" + current.getDistance() + ")",
                                      new ImageIcon(new BufferedImage(200, 200, BufferedImage.TYPE_INT_ARGB)),
                                      JLabel.CENTER);
